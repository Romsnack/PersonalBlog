...
summary: Ce qu'est réellement une image sur le disque, pourquoi un RUN rm ne supprime jamais rien, et trois façons de ressortir des identifiants d'une couche qu'un conteneur en cours d'exécution jure pourtant vide.
...
comment cette clé a-t-elle atterri sur GitHub » cesse d'être mystérieuse.

En résumé : **les couches sont append-only**. Supprimer un fichier écrit une
nouvelle couche qui dit « ignore ce fichier », et la couche qui contient les
octets d'origine reste exactement où elle était, livrée à tous ceux qui font
un `pull` de l'image.
...
Ce masquage, c'est toute l'astuce. Quand un `RUN rm` supprime un fichier, le
...
Le voilà, en clair, dans des métadonnées livrées avec l'image. Aucun
dépaquetage nécessaire, et `docker history` fonctionne sur une image récupérée
depuis un registre. **Un argument de build n'est pas un secret.** C'est un
commentaire agrafé à votre image, avec la valeur écrite dedans.

C'est la vérification la moins chère qui soit et elle coûte une commande : elle
a sa place dans votre pipeline, que vous fassiez ou non le reste de cet article.

## Attaque 2 : docker save et un peu de fouille

`docker history` ne voit que les métadonnées. Pour le contenu des fichiers, il
faut les couches elles-mêmes, et `docker save` diffuse l'image entière sous
forme de tar :
...
**gzippées** — `grep` scanne des octets compressés. Ce faux négatif mérite
d'être intériorisé, car c'est exactement le résultat qui pousse à déclarer une
image saine. Il faut décompresser d'abord :
...
Listez cette couche face à celle empilée juste au-dessus, et toute l'histoire
est là :
...
`rm` a produit une pierre tombale, pas une suppression.
...
existe dans une couche et est supprimé dans une couche ultérieure » est
exactement la forme d'un identifiant qui fuit**, ce qui fait d'un outil
d'efficacité un détecteur étonnamment bon.

Si vous ne voulez pas installer dive en local, il tourne depuis sa propre
image :
...
a un jour commités puis annulés.
...
**Utilisez des builds multi-étapes.** Faites le travail qui nécessite des
identifiants dans une étape de build, puis `COPY --from=builder` uniquement
l'artefact. Les couches dont vous ne copiez rien n'atteignent jamais l'image
finale.

Reconstruite avec les trois correctifs, la même analyse revient vide — aucun
`.env` dans la moindre couche, et l'historique enregistre le chemin de montage
plutôt que le jeton :
...
Faites tourner l'identifiant. Ensuite, reconstruisez.

Voilà tout : les images sont append-only, `rm` est une pierre tombale, et trois
commandes vous diront ce que vous avez réellement livré.
